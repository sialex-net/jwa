...
export async function getUserId(request: Request, env: Env) {
...
async function verifyUserPassword(
	...
	let userWithPassword = await db
		...
		.from(schema.users)
		.where(eq(schema.users.email, email.email))
	...
	const isValid = await compare(password, userWithPassword.hash);
